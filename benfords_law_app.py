...
# Page configuration
st.set_page_config(
    page_title="Benford's Law Audit Tool",
    layout="centered",
    page_icon="portfolio.ico"
)
...
if uploaded_file:
    ...
    if not numeric_cols:
        ...
        selected_col = st.selectbox(
            "Select the column to test with Benford's Law",
            numeric_cols
        )
        ...
        first_digits = [
            int(str(int(abs(x)))[0]) for x in values if x != 0
        ]
        ...
        actual_dist = {
            d: actual_counts.get(d, 0) / total for d in digits
        }

        # Benford's expected distribution
        benford_dist = {d: np.log10(1 + 1 / d) for d in digits}

        # Lists for plotting and export
        ...
        st.download_button(
            ...
            data=buf,
            file_name=f"benfords_{selected_col}.png",
            mime="image/png"
        )
        ...
        **p-value**: `{p_value:.4f}`  
        ...
        # --- Build results table for CSV export ---
        results_df = pd.DataFrame({
            "Digit": digits,
            "Actual Frequency": actual,
            "Expected Frequency (Benford)": expected,
            "Actual Count": observed,
            "Expected Count": expected_counts,
        })

        st.subheader("Benford Results Table")
        st.dataframe(results_df.style.format({
            "Actual Frequency": "{:.4f}",
            "Expected Frequency (Benford)": "{:.4f}",
            "Expected Count": "{:.2f}"
        }))
        ...
        st.download_button(
            ...
            data=csv_buf,
            file_name=f"benfords_{selected_col}_results.csv",
            mime="text/csv"
        )
